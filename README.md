...
- **分类链接看板**：链接按媒体、AI、开发工具、社交、搜索 5 大类分列展示，卡片自动加载网站图标（多级容错兜底：原站图标 → DuckDuckGo Favicon 镜像 → 首字母色块）
- **实时搜索与拼音/别名匹配**：按 `/` 或 `Ctrl+K` 快速聚焦搜索框，支持名称 / URL / 分类名模糊匹配及拼音/缩写/别名检索（如 `bili`、`zh`、`gh`、`ds` 等）；命中关键词高亮并显示结果数量；历史下拉支持 `↑↓` 选择、`Enter` 搜索、`Esc` 关闭，兼容中文输入法
...
- **侧边栏系统面板**：
  - 实时时钟（时:分:秒 + 日期 + 秒级平滑周进度条，Orbitron 数字字体）
...
- **视觉特效**：背景网格与顶部光晕、GPU 合成层优化、SVG 噪点纹理、卡片错峰进场、卡片悬停光晕、自定义滚动条与文本选择配色
...
| 文件 | 说明 |
|------|------|
| `index.html` | **CYBER-DECK V2.14.5**，GitHub Pages 默认首页，链接数据内联，可直接 `file://` 打开 |
| `index_v3.html` | **CYBER-DECK V2.14.5 · YAML**，运行时加载 `links.yaml`，失败时回退内置 `fallbackConfig` |
