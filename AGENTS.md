...
- Keep shared localStorage keys using the `v14` namespace: `cyber-v14-theme`, `v14-link-order`, `cyber-v14-weather`, `v14-memos`, `v14-search-history`, and `cyber-v14-periph-*`.
- Version/name convention: current release marker is `V2.14.15` (title `CYBER-DECK V2.14.15`; the YAML entry appends `· YAML`). Each small change increments ONLY the last digit (patch) automatically — e.g. `V2.14.15` → `V2.14.16`. The major/minor digits (`2.14`) are user-governed and must NOT be changed unless the user says so. Keep exactly one authoritative version marker, written in three places: `<title>` of both entries, the `.version-badge` in the sidebar (above `SECURE_TIME`), and `README.md`. The badge text and `<title>` must stay in sync. When the version marker here becomes stale relative to the actual release, update this line to match.
