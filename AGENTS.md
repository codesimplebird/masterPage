...
- Version/name convention: current release marker is `V2.14.3` (title `CYBER-DECK V2.14.3`; the YAML entry appends `· YAML`). Each small change increments ONLY the last digit (patch) automatically — e.g. `V2.14.3` → `V2.14.4`. The major/minor digits (`2.14`) are user-governed and must NOT be changed unless the user says so. Keep exactly one authoritative version marker, written in three places: `<title>` of both entries, the `.version-badge` in the sidebar (above `SECURE_TIME`), and `README.md`. The badge text and `<title>` must stay in sync.
